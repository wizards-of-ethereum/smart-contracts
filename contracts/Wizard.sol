...
contract Wizard {
    ...
    function getExpansions()
        public
        view
        returns (address[] memory _expansions)
    {
        return existingExpansions;
    }

    function newLegend(address _expansion, string memory uri)
        public
        returns (uint256 newItemId)
    {
        require(
            expansions[_expansion] != address(0),
            "Expansion does not exists"
        );
    ...
    function ownerOf(address _expansion, uint256 _tokenId)
        public
        view
    ...
    function tokenURI(address _expansion, uint256 _tokenId)
        public
        view
